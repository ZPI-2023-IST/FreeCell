...
class FreeCell(Game):
    ...
    def get_all_moves(self) -> list:
        ...
        :notation: A move is a tuple of the form (card, destination), where:
            ...
            * destination is either:
                - 'F' for freecell
                - 'S' for suit stack
                - '{rank}{color}'card for any other destination card.
                - '0' for empty columns
            * examples:
        ...
        if not None in self.board.free_cells:
        ...
        for card in self.board.free_cells:
            if card:
                # Check if any card from freecells can be moved onto a suit stack
        ...
        for card in suspected_moves:
            ...
            for card_destination in suspected_moves:
                if card != card_destination and card.is_smaller_and_opposite_color(card_destination):
                    moves.append((str(card), str(card_destination)))
    ...
    def get_board(self) -> list:
        ...
        :return: The current board state as a list of 10 lists:
            ...
            * The last 4 element long list is the list of the top cards on each suit stack.
        """
        return self.board.columns + self.board.free_cells + list(self.board.suit_stack.values())